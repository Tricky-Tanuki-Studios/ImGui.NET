...
There is obviously *a lot* more broken than what is shown, this is just what I was able to catch and need to note for
  ...
  - Fix broken types in [src/ImGui.NET/Generated/ImGuiNative.gen.cs](src/ImGui.NET/Generated/ImGuiNative.gen.cs). This
    needs to be fixed on a generator level, ***not*** by fucking hand, it should never be done by hand. Fix it in the
    generator.
  ...
  - Missing/broken types
    - Font related stuff [this one might be harder to figure out due to all of the v1.92 changes with fonts]
      - [ ] `ImDrawTextFlags`
        - Spotted being used at [src/ImGui.NET/Generated/ImFont.gen.cs](src/ImGui.NET/Generated/ImFont.gen.cs)
      - [ ] `ImFontAtlasBuilder`
        - Spotted being used
